...
func TestBlackList_AddIP(t *testing.T) {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", "/auth/blacklist", bytes.NewReader(body))
	require.NoError(t, err)
...
func TestBlackList_RemoveIP(t *testing.T) {
	...
	ctx := context.Background()
	// Отправляем POST-запрос на сервер для добавления IP в черный список
	req, err := http.NewRequestWithContext(ctx, "DELETE", "/auth/blacklist", bytes.NewReader(body))
	require.NoError(t, err)
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()
	router.ServeHTTP(rr, req)
...
func TestBlackList_ShowIPList(t *testing.T) {
	...
	req, err := http.NewRequest("GET", ts.URL+"/auth/blacklist", nil)
	if err != nil {
		t.Fatal(err)
	}
